const CardExperience = ({ experience }) => {
  return (
    <article className="gap-5 grid text-primary-dark dark:text-secondary dark:bg-gray-50/5 rounded-xl border dark:border-gray-50/10 p-8 duration-200 card-hover dark:hover:border-primary">
      <div className="flex justify-start flex-col md:flex-row md:justify-between items-start md:items-center font-semibold text-xl">
        <div className="flex items-start font-semibold flex-col text-secondary dark:text-white justify-start">
          <h3 className="!text-xl text-foreground">{experience.job}</h3>
          <h4 className="text-primary text-sm">{experience.company}</h4>
        </div>

        <div className="text-xs font-medium space-x-2">
          <span className="border px-2 py-0.5 text-forty border-forty rounded-full">
            {experience.date}
          </span>
          {experience.currentJob && (
            <span className="px-2 text-black border border-primary py-0.5 bg-primary rounded-full">
              Actual
            </span>
          )}
        </div>
